...
app = FastAPI(title="Alam Enterprise Cloud Sync Server")

# Enable CORS so that anyone can connect
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def load_db():
    if not os.path.exists(DB_FILE):
        return {
            ...
            "returns": [],
            "staff": [],
            "last_updated": "কখনো সিঙ্ক করা হয়নি"
        }
    try:
        ...
        return {
            ...
            "returns": [],
            "staff": [],
            "last_updated": "ত্রুটি ঘটেছে"
        }

def save_db(data):
    data["last_updated"] = datetime.now().strftime("%d %B %Y, %I:%M:%S %p")
...
async def sync_post(request: Request):
    try:
        ...
        return {
            "status": "success", 
            "message": "ডাটা ক্লাউডে সফলভাবে সিঙ্ক হয়েছে", 
...
async def dashboard():
    ...
    last_updated = db.get("last_updated", "কখনো সিঙ্ক করা হয়নি")

    # Business Calculations
    try:
        total_sales = sum(float(i.get("totalAmount", 0)) for i in invoices)
    except:
        total_sales = 0.0

    try:
        total_cash_collected = sum(float(i.get("paidAmount", 0)) + float(i.get("duePaymentAmount", 0)) for i in invoices)
    except:
        total_cash_collected = 0.0

    try:
        total_dues = sum(float(c.get("previousDues", 0)) for c in customers)
    except:
        total_dues = 0.0

    try:
        low_stock_count = sum(1 for p in products if int(p.get("quantity", 0)) <= int(p.get("lowStockCount", 5)))
    except:
        low_stock_count = 0
        ...
        <div class="max-w-6xl mx-auto space-y-6">
            ...
            <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p class="text-slate-400 text-sm mt-1">SR অর্ডার ট্র্যাকিং ও অ্যাডমিন ক্লাউড ড্যাশবোর্ড</p>
                </div>
